"""theta_G FIR/filter-parameter optimization core for Exp1.
...
2. Call ``theta_G_cvx.solve_step2_cvx_min`` to solve the convex theta_G
   subproblem. This corresponds to arXiv v2 Eq. (15), with the regularized
   objective related to Eq. (14).
...
4. Run open-loop and closed-loop rollouts with the solved FIR bank, using the
   NFIR branch structure of arXiv v2 Eq. (7)-Eq. (10).
...
from . import io_data
from .theta_N_core import SharedMLP

from . import theta_G_cvx
...
def build_default_config_min(
    ...
    # iter0 is the base behavior; n_refine_iter adds extra solve/simulate rounds.
    ...
    """Refinement preprocessing note:
        iter_yhat_add_noise and iter_yhat_lpf_enable also apply to the
        initial p_t when rebuild_p7_from_uy=True, even if n_refine_iter=0.
    """
...
def _validate_cfg(cfg: dict[str, Any]) -> None:
    ...
    # Validate iterative refinement settings:
    # - n_refine_iter >= 0
    # - iter_p7_ts > 0
...
def _load_inputs_random(cfg: dict[str, Any]) -> tuple[dict[str, Any], int, int]:
    ...
    # Convert to model input format (B,T,F), float32 to match theta_N/legacy theta_G path.
...
def _build_poly_k_jtb_from_x_all(
    ...
    This is the deterministic polynomial alternative to the neural lifting
    functions in arXiv:2508.05279v2 Eq. (8) and Eq. (20). The degree-0 term
    gives k(t)=1, which reduces the lifted branch to a pure FIR contribution.
    """
...
def _load_inputs_polylift(cfg: dict[str, Any]) -> tuple[dict[str, Any], int, int]:
    ...
    # Use the shared polynomial-basis helper so ordering is reused when
    # rebuilding k_jtb inside iterative refinement.
    ...
    inputs["feature_std"] = np.asarray(feature_std_f, dtype=float)
    inputs["x_max"] = cfg.get("x_max", None)
    # Store optional NN keys for consistency across modes.
...
def _p_7tb_regen(
    ...
    """Regenerate p_7tb from u_tb and y_hat_tb.

        Shape contracts:
        - Input: u_tb(T,B), y_hat_tb(T,B), ts (float scalar)
        - u_tb and y_hat_tb cover all available batches, not only train/validation splits.
        - Output: p_7tb shape (7,T,B)

        Note:
        - B is the total number of provided batches, not the number of CVX training batches.
...
def _rebuild_k_jtb_all_mode(inputs:dict[str, Any], cfg_local:dict[str, Any], p_7tb_new: np.ndarray) -> np.ndarray:
    """Rebuild k_jtb from regenerated p_7tb for all k-source modes.
...
def _compute_constraint_diagnostics(
    ...
    passivity_matrix = theta_G_cvx.build_passivity_matrix(m_count, int(ms_passivity))
...
def _mse_on_split(y_hat_tb: np.ndarray, y_tb: np.ndarray, split_idx: np.ndarray) -> float:
    """
    ...
    err = np.asarray(y_hat_tb[:, idx] - y_tb[:, idx], dtype=float)
...
def _simulate_collect_closed_loop(inputs: dict[str, Any], g_jm: np.ndarray, cfg_local: dict[str, Any]) -> dict[str, Any]:
    """
    ...
    train_mse = _mse_on_split(y_hat_tb, y_tb, split_train_idx)
    val_mse = _mse_on_split(y_hat_tb, y_tb, split_val_idx)
    test_mse = _mse_on_split(y_hat_tb, y_tb, split_test_idx)
...
def _simulate_collect(inputs: dict[str, Any], g_jm: np.ndarray) -> dict[str, Any]:
    """
    ...
    # print(' ')
    # timer1 = time.time()
    y_hat_tb, y_branch_jtb, s_jtb, v_jtb, g_oim = theta_G_mimo.nfir_forward_diagonal_mimo(
        k_jtb=k_jtb,
        u_tb=u_tb,
...
def run_step2_min(cfg: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if mode_text == "imported_nn":
        if cfg_local.get("n_branch", None) is None or cfg_local.get("m_fir", None) is None:  
            # Fill imported-NN dimensions from the theta_N result when not provided in cfg.
    ...
    if bool(cfg_local["verbose_solver"]):
        ...
        # Pre-CVX logging includes iteration settings:
        # - n_refine_iter
    ...
    for iter_idx in range(0, iter_idx_max + 1):
        ...
        solve_out = theta_G_cvx.solve_step2_cvx_min(
            u_tb=u_tb_arr,
            y_tb=y_tb_arr,
            k_jtb=k_jtb_arr,
    ...
    # Store result["iter_history"] with one dict per iteration:
    ...
    # Keep NN weights available in the theta_G result for analytic/debug tests.
    # Shape detail:
