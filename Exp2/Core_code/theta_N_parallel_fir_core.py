...
import time
from Exp2.Core_code import theta_G_parallel_fir_mimo as parallel_mimo
...
class SharedMLP(nn.Module):
    ...
    def forward(self, x_btf: torch.Tensor) -> torch.Tensor:
        """
          ...
          Input:
          - x_btf: (B, T, F), extended scheudling signal/feature
        ...
        # Flatten (B,T,F) -> (B*T,F) to evaluate all batches in one call.
...
def build_default_config() -> dict[str,Any]:
    ...
    cfg["scale_io_by_20"] = False # Optional legacy input/output scaling.
    ...
    # Logging settings.
    cfg["verbose"] = True # Print training progress.
    ...
    # "none" keeps the previous fixed-learning-rate behavior.
...
def _load_parallel_fir_from_step2_pkl(step2_pkl_path: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    if "g_linear_m" not in step2_dict:
        raise ValueError(f"Step2 pickle missing g_linear_m: {step2_path_obj}")

    g_bank_jm = np.asarray(step2_dict["g_bank"], dtype=float)  # shape (J,M)
    g_linear_m = np.asarray(step2_dict["g_linear_m"], dtype=float).reshape(-1)  # shape (M,)

    if g_bank_jm.ndim != 2:
        raise ValueError("g_bank must have shape (J,M)")
    if g_linear_m.shape[0] != g_bank_jm.shape[1]:
        raise ValueError("g_linear_m length must match FIR length M")

    return g_bank_jm, g_linear_m
...
def causal_per_branch_fir(ku_btj: torch.Tensor, g_jm: torch.Tensor) -> torch.Tensor:
    ...
    - Same operation as previous implementation:
...
def nfir_forward(model: SharedMLP, x_btf: torch.Tensor, u_bt: torch.Tensor, g_jm: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    # First evaluate NN output
    # print('MAYBE A BUG HERRRE')
    # print('MAYBE A BUG HERRRE')
    # print('MAYBE A BUG HERRRE')
    # print('MAYBE A BUG HERRRE')
    # print('MAYBE A BUG HERRRE')
    ...
    # Compute K(feature)*u,  No time to figure out the syntix here 
...
def _eval_mse(model: SharedMLP, x_btf: torch.Tensor, u_bt: torch.Tensor,
               y_bt: torch.Tensor, g_jm: torch.Tensor,
               zero_cost_first_n: int = 0) -> float:
    """
    ...
    t_count = int(y_bt.shape[1])  # scalar T
    if n_free < 0 or n_free >= t_count:
        raise ValueError("zero_cost_first_n must satisfy 0 <= N < T")

    # I now want the NN temporaly go into a mode efficeint for 
    # evaluation. But I want to close this mode immediately
    # once I got what i want
    with torch.no_grad(): # turn on .no_grad() mode in the following block and turn it off afterwards
        ...
        # Compute scalar MSE.
        mse_tensor = F.mse_loss(y_hat_bt[:, n_free:], y_bt[:, n_free:])
...
def bptt_closed_loop_rollout(
   ...
   g_linear_m: torch.Tensor,
    ...
    """
        ...
        - g_jm: torch.Tensor, shape (J,M)
        - g_linear_m: torch.Tensor, shape (M,)
    ...
    dtype = u_bt.dtype
    dt_sec = cfg["dt"]
    g_linear_m = g_linear_m.to(device=device, dtype=dtype).reshape(M)  # shape (M,)
    u_linear_hist_bm = torch.zeros(B, M, dtype=dtype, device=device)  # shape (B,M)
    ...
    for t_index in range(T):
        ...
        if M > 1:
            u_linear_hist_bm = torch.cat(
                [u_bnow.reshape(B, 1), u_linear_hist_bm[:, 0:M - 1]],
                dim=1,
            )  # shape (B,M)
        else:
            u_linear_hist_bm = u_bnow.reshape(B, 1)  # shape (B,M=1)
        ...
        if M > 1:
            s_hist_bjm = torch.cat([s_current_bj.reshape(B, J, 1), s_hist_bjm[:, :, 0:M - 1]], dim=2)# shape history horizontal by one step to discard previous memory
        else: 
            s_hist_bjm = s_current_bj.reshape(B,J,1) # since there is no memeory/history. so history is just current
        ...
        y_linear_b = torch.sum(u_linear_hist_bm * g_linear_m.reshape(1, M), axis=1)  # shape (B,)
        y_nfir_b = torch.sum(y_branch_bj, axis=1)      
        y_hat_b = y_nfir_b + y_linear_b  
...
def _infer_all_closed_loop(
    ...
    """
        ...
        - y_nfir_tb_cl: np.ndarray, shape (T,B)
        - y_linear_tb_cl: np.ndarray, shape (T,B)
    """
    model.eval()

    g_linear_arr = np.asarray(g_linear_m, dtype=float).reshape(-1)  # shape (M,)
    if g_linear_arr.shape[0] != int(g_jm.shape[1]):
        raise ValueError("g_linear_m length must match FIR length M")
    ...
    y_nfir_tb_cl = np.zeros((t_count, b_count), dtype=float)    # shape (T,B)
    y_linear_tb_cl = np.zeros((t_count, b_count), dtype=float)  # shape (T,B)
    ...
    for b_index in range(b_count):
        ...
        for t_index in range(t_count):
            ...
            y_nfir_tb_cl[t_index, b_index] = y_nfir_now
            y_linear_tb_cl[t_index, b_index] = y_linear_now

    return y_hat_tb_cl, y_branch_jtb_cl, k_jtb_cl, p_7tb_cl, y_nfir_tb_cl, y_linear_tb_cl
...
def train_step1(u_tb: np.ndarray, y_tb: np.ndarray, p_7tb: np.ndarray,
    ...
    cfg.setdefault("enable_parallel_fir", True)
    cfg.setdefault("zero_cost_first_n", 0)
    if str(cfg["fir_source_type"]).strip().lower() != "step2_pkl":
        raise ValueError("parallel FIR Step1 requires fir_source_type='step2_pkl'")

    # First, we double check the cfg for NN is correct
    _validate_config(cfg)
    
    # Next, we fix the seeds for NN training 
    np.random.seed(int( cfg["random_seed"]  ))
    torch.manual_seed(  int(  cfg["random_seed"]   )   )

    # Double check training data format 
    u_tb = np.asarray(u_tb, dtype=float)
    y_tb = np.asarray(y_tb, dtype=float)
    p_7tb = np.asarray(p_7tb, dtype=float)

    # Double check traing data dimensions.
    ...
    n_free = int(cfg["zero_cost_first_n"])  # scalar int
    ...
    g_bank_jm, g_linear_m = _load_parallel_fir_from_step2_pkl(step2_pkl_path_text)
    if not bool(cfg["enable_parallel_fir"]):
        g_linear_m = np.zeros((int(g_bank_jm.shape[1]),), dtype=float)  # shape (M,)
    
    y_linear_tb = parallel_mimo.linear_fir_forward(
    u_tb=u_tb,
    g_linear_m=g_linear_m,
    )  # shape (T,B)
    y_resid_tb = y_tb - y_linear_tb  # shape (T,B)
    ...
    y_all_bt = np.transpose(y_resid_tb, (1, 0)).astype(np.float32)  # shape (B,T)
    ...
    g_bank_loaded_shape_jm = (int(g_bank_jm.shape[0]), int(g_bank_jm.shape[1]))
    ...
    g_linear_t = torch.tensor(g_linear_m, dtype=torch.float32, device=device)  # shape (M,)
    ...
    best_train_loss = np.inf # scalar, best objective cost for training data
    best_val_loss = np.inf # scalar, validation loss at the training-best epoch
    best_epoch = -1 # which iteration/epoch gives the best results
    bad_count = 0 # incre this by 1 if one epoch does not improve compared with previous epoch

    # Log  
    history_epoch = []
    history_train = [] # for train signals 
    history_val = [] # for valiation signals 
    history_test = [] # for test signals
  
    # Double make sure train index foramt
    ...
    if not skip_open_loop_training:
        ...
        for epoch in range(1, int(cfg["max_epochs"]) + 1):
            ...
            if bool(cfg["deterministic_epoch_shuffle"]):
                rng_epoch = np.random.default_rng(epoch + int( cfg["split_seed"] ) ) # so we choose a different seed! 
                rng_epoch.shuffle(epoch_perm) # shuffle with the new created seed
            ...
            for start in range(0, len(epoch_perm), mini_batch_size): # range(start, stop, stepsize)
                ...
                # Compute the loss using built-in tools, 
                # this is now a symbolic cost function!!!
                loss = F.mse_loss(y_hat_b[:, n_free:], yb[:, n_free:]) # import torch.nn.functional as F

                # Compute grad of the symolic function using built-in backpropogation
                # grad is stored/updated inside model.parameters()
                loss.backward()

                # clip the grad if it is too large 
                if cfg["grad_clip_norm"] is not None:
                    nn.utils.clip_grad_norm_(model.parameters(), float(cfg["grad_clip_norm"]))

                # Perform parameter update using specifed optimiser
                optimizer.step()
            
            # Now all mini-batches have been trained, we record NN performance
            tr_loss = _eval_mse(model, x_t[tr_idx], u_t[tr_idx], y_t[tr_idx], g_t, n_free)
            va_loss = _eval_mse(model, x_t[va_idx], u_t[va_idx], y_t[va_idx], g_t, n_free)
            te_loss = _eval_mse(model, x_t[te_idx], u_t[te_idx], y_t[te_idx], g_t, n_free)
            ...
            # Check if training loss improves for this epoch
            improved = tr_loss < (best_train_loss - float( cfg["early_stopping_min_delta"]  ))
            if improved:
            ...
            if bool(cfg["verbose"]):
                ...
                if should_log:
                    ...
                    print(
                        ...
                        f"| best_train={best_train_loss:.6f} "
                        f"| val_at_best_train={best_val_loss:.6f} @ {best_epoch}"
                    )
        ...
        best_train_loss = _eval_mse(model, x_t[tr_idx], u_t[tr_idx], y_t[tr_idx], g_t, n_free)
        best_val_loss = _eval_mse(model, x_t[va_idx], u_t[va_idx], y_t[va_idx], g_t, n_free)
        if bool(cfg["verbose"]):
    ...
    bptt_history_lr = []
    bptt_best_epoch = -1 
    bptt_best_train_loss = float("inf")
    ...
    if bool(cfg["bptt_finetune_enable"]) and (int(cfg["bptt_max_epochs"]) > 0):
        # This guarantees previous behavior is unchanged unless the user explicitly enables BPTT.
        ...
        with torch.no_grad():
            ...
            bptt_tr_loss = F.mse_loss(y_hat_tra_bt_cl_torch[:, n_free:], y_bt_torch[tr_idx][:, n_free:]).item()
            bptt_va_loss = F.mse_loss(y_hat_val_bt_cl_torch[:, n_free:], y_bt_torch[va_idx][:, n_free:]).item()
            bptt_te_loss = F.mse_loss(y_hat_tes_bt_cl_torch[:, n_free:], y_bt_torch[te_idx][:, n_free:]).item()
        ...
        bptt_best_epoch = 0
        bptt_best_train_loss = bptt_tr_loss
        ...
        if bool(cfg["verbose"]):
            print(
                ...
                f"| best_train={bptt_best_train_loss:.6f} "
                f"| val_at_best_train={bptt_best_val_loss:.6f} @ {bptt_best_epoch}"
        ...
        for epoch in range(1, int(cfg["bptt_max_epochs"]) + 1):
            ...
            for start in range(0, len(bptt_epoch_perm), mini_batch_size): # range(start, stop, stepsize)
                ...
                # Compute the loss using built-in tools, 
                # this is now a symbolic cost function!!!
                loss = F.mse_loss(y_hat_minibt_cl_torch[:, n_free:], y_minibt_torch[:, n_free:]) # import torch.nn.functional as F
            ...
            with torch.no_grad():
                ...
                # use .item to return as numpy array 
                bptt_tr_loss = F.mse_loss(y_hat_tra_bt_cl_torch[:, n_free:], y_bt_torch[tr_idx][:, n_free:]).item() # import torch.nn.functional as F
                bptt_va_loss = F.mse_loss(y_hat_val_bt_cl_torch[:, n_free:], y_bt_torch[va_idx][:, n_free:]).item() # import torch.nn.functional as F
                bptt_te_loss = F.mse_loss(y_hat_tes_bt_cl_torch[:, n_free:], y_bt_torch[te_idx][:, n_free:]).item() # import torch.nn.functional as F
        

            if bptt_scheduler is not None and bptt_scheduler_name == "plateau":
                bptt_scheduler.step(bptt_tr_loss)
            ...
            # Check if training loss improves for this epoch
            improved = bptt_tr_loss < (bptt_best_train_loss - float( cfg["early_stopping_min_delta"]  ))
            if improved:
                bptt_best_train_loss = bptt_tr_loss
                bptt_best_val_loss = bptt_va_loss
        ...
        #All epoch training finishes. Restore best model state
    ...
    y_nfir_bt, y_branch_btj, k_btj = _infer_all(model, x_t, u_t, g_t)

    y_nfir_tb = np.transpose(y_nfir_bt, (1, 0))  # shape (T,B)
    ...
    train_mse_ol = float(np.mean((y_pre_train_tb[n_free:, :] - y_train_tb[n_free:, :]) ** 2))
    val_mse_ol = float(np.mean((y_pre_val_tb[n_free:, :] - y_val_tb[n_free:, :]) ** 2))
    test_mse_ol = float(np.mean((y_pre_test_tb[n_free:, :] - y_test_tb[n_free:, :]) ** 2))

    train_mse_cl = float(np.mean((y_pre_train_tb_cl[n_free:, :] - y_train_tb[n_free:, :]) ** 2))
    val_mse_cl = float(np.mean((y_pre_val_tb_cl[n_free:, :] - y_val_tb[n_free:, :]) ** 2))
    test_mse_cl = float(np.mean((y_pre_test_tb_cl[n_free:, :] - y_test_tb[n_free:, :]) ** 2))
    ...
    result["zero_cost_first_n"] = int(n_free)
    ...
    result["best_train_loss"] = float(best_train_loss)
    ...
    result["y_linear_tb"] = np.asarray(y_linear_tb, dtype=float)        # shape (T,B)
    result["y_nfir_tb"] = np.asarray(y_nfir_tb, dtype=float)            # shape (T,B)
    result["y_resid_tb"] = np.asarray(y_resid_tb, dtype=float)          # shape (T,B)

    result["y_hat_tb_cl"] = np.asarray(y_hat_tb_cl, dtype=float)
    result["y_nfir_tb_cl"] = np.asarray(y_nfir_tb_cl, dtype=float)          # shape (T,B)
    result["y_linear_tb_cl"] = np.asarray(y_linear_tb_cl, dtype=float)      # shape (T,B)
    ...
    result["bptt_best_train_loss"] =  float(bptt_best_train_loss)
