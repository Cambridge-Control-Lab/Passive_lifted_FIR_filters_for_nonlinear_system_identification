...
Minimal data input/output helpers for the NFIR Step1 rewrite.

Design intent:
- Keep only what is needed for the current Step1 training path.
- Keep code explicit and easy to read for engineering use.
- State dimensions carefully in every function.
"""
...
def load_training_mat(mat_path: str | Path) -> dict:
    ...
    # Slice and cast to get the exact arrays used by prior Step1 path.
    # u_tb has shape (T, B).
...
def save_step1_outputs(result: dict, out_dir: str | Path, run_name: str) -> tuple[Path, Path]:
    ...
    - Field names follow Step1 export naming for compatibility.
    """
    ...
    # Build MATLAB export structure with compatibility-preserving field names.
    data_mat = {}
