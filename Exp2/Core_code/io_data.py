...
MATLAB/Python data I/O helpers for Exp2 NFIR runs.
...
def load_training_mat(mat_path: str | Path) -> dict:
    ...
    # Slice and cast to get the exact arrays used by the prior theta_N path.
    # u_tb has shape (T, B).
...
def save_step1_outputs(result: dict, out_dir: str | Path, run_name: str) -> tuple[Path, Path]:
    ...
    - Field names follow theta_N export naming for compatibility.
    """
