"""
Exp2 NFIR alternating-training orchestration script.

Role in the workflow:
- This script runs the Exp2 outer loop for Algorithm 1 of
  arXiv:2508.05279v2.
...
- theta_G writes a ``*.pkl`` and ``*_train.mat`` result containing the solved
  standalone linear FIR g_linear_m, shape (M,), and NFIR branch bank g_jm,
  shape (J,M).
...
Execution policy:
- each theta_G/theta_N stage runs exactly once per outer iteration
...
EXP2_DIR = Path(__file__).resolve().parents[1]
...
from Exp2.Core_code import theta_N_parallel_fir_core as train_core
...
is_passive = True
step2_l2reg = 1.0
step1_weight_decay = 1e-5
enable_parallel_fir = True
...
parser.add_argument("--step1-weight-decay", type=float, default=step1_weight_decay)
parser.add_argument("--enable-parallel-fir", type=int, choices=[0, 1], default=1)
parser.add_argument("--last-bptt-on", type=int, choices=[0, 1], default=0)
parser.add_argument("--step1-epoch-count", type=int, default=50)
parser.add_argument("--zero-cost-first-n", type=int, default=0)
...
step1_weight_decay = float(args.step1_weight_decay)
enable_parallel_fir = bool(args.enable_parallel_fir)
last_bptt_on = bool(args.last_bptt_on)
step1_epoch_count = int(args.step1_epoch_count)
if step1_epoch_count < 1:
    raise ValueError("step1_epoch_count must be >= 1")
zero_cost_first_n = int(args.zero_cost_first_n)
if zero_cost_first_n < 0:
...
l2nn_tag = f"l2NN{step1_weight_decay:g}".replace(".", "p")
fir_parallel_tag = "firParallOn" if enable_parallel_fir else "firParallOff"
step1_epoch_tag = f"ep{step1_epoch_count}"
zc_tag = f"zc{zero_cost_first_n}"
case_tag = f"run_Exp2_{exp_id}_h{hidden_dims[0]}_ad{active_dim_tag}_m{m_fir}_b{n_branch_all_cases}_{noise_tag}_{passive_tag}_ite{max_iteration}_{bptt_tag}_{l2_tag}_{l2nn_tag}_{fir_parallel_tag}_{step1_epoch_tag}_{zc_tag}_neoit"
...
for iter_idx in range(0,max_iteration):
    ...
    cfg_step2["enable_parallel_fir"] = enable_parallel_fir
    cfg_step2["zero_cost_first_n"] = zero_cost_first_n
    ...
    cfg_step2["train_val_test_split"] = (1, 1, 2) 
    ...
    cfg_step1["enable_parallel_fir"] = enable_parallel_fir
    cfg_step1["zero_cost_first_n"] = zero_cost_first_n
    ...
    cfg_step1["max_epochs"] = step1_epoch_count
    cfg_step1["early_stopping_patience"] = step1_epoch_count
    ...
    cfg_step1["train_val_test_split"] = (1, 1, 2) 
    cfg_step1["batch_size"] = 1
    ...
    cfg_step1["dt"] = dt
    cfg_step1["weight_decay"] = step1_weight_decay

    if last_bptt_on and iter_idx == max_iteration - 1:
    # if last_bptt_on:     
        cfg_step1["skip_open_loop_training"] = True 
        cfg_step1["bptt_finetune_enable"] = True  
        cfg_step1["bptt_max_epochs"] = 20
        cfg_step1["bptt_learning_rate"] = 5e-5
        cfg_step1["bptt_batch_size"] = 1
        cfg_step1["bptt_early_stopping"] = False
        cfg_step1["bptt_early_stopping_patience"] = 20
