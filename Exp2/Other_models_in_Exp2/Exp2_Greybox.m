...
training_data_dir = fullfile(exp2_dir, 'Training_data');
...
% identification experiments detailed below are based on the work published
% in
%
...
% q_a(t), are non-measurable. Flexibilities within the gear-box are modeled
...
% Fcs and alpha are coefficients for reflecting the Stribeck effect, and
...
%%%%%%%%%%%%%%%%%%%%%%%% modification start %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
smatalb = settings;
% Note: whenever model data, use struc! k can be spring stiffness 
% but can also be index in a for loop....
...
% IDDATA object z:
load(fullfile(training_data_dir, 'robotarmdata.mat'));

%%%%%%%%%%%%%%%%%%%%%%%% modification start %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
z_2KHz = iddata({ye yv1 yv2 yv3}, {ue uv1 uv2 uv3}, 0.5e-3, 'Name', 'Robot arm');
%%%%%%%%%%%%%%%%%%%%%%%% modification end %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

z = idresamp(z_2KHz,[1 10]);
...
% initial states, and with z containing four different experiments this would
...
% estimation.
Ns = size(zi,1);   % Jeff: Ns = 1984, num of data points. zi is experiment 4
zred = z(1:round(Ns/10)); % Jeff: zred: iddata object with 4 experiments, but only first 198 samples each
nlgr = setinit(nlgr, 'Fixed', {true true false false false}); % Jeff:  Set the last three initial states free to estimate
X0 = nlgr.InitialStates; %Jeff: X0: 5-by-1 structure array with fields: Name unit Value Minium Maximum Fixed
[X0.Value] = deal(zeros(1, 4), zeros(1, 4), [ye(1) yv1(1) yv2(1) yv3(1)], ...
    [ye(1) yv1(1) yv2(1) yv3(1)], [ye(1) yv1(1) yv2(1) yv3(1)]);
[~, X0init] = predict(zred, nlgr, [], X0); % X0init 5x4 matrix 
...
X0init(:, 1) = cell2mat(getinit(nlgr, 'Value')); %Jeff: X0init(:, 1): initial state for experiment 1
...
%%%%%%%%%%%%%%%%%%%%%%%% modification start %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
z1 = getexp(z, 1);                 % experiment 1 only
% x01_zero = zeros(5,1);               % 5-by-1 initial state for experiment 1
x01_esti = X0init(:, 1);
ysim1 = sim(nlgr, z1.InputData, simOptions('InitialCondition', x01_esti));
[metric_fit_train,metric_mse_train] = helper_func_new_metrics_by_traj(z1.OutputData, ysim1);


z2 = getexp(z, 2);                 % experiment 2 only
x02_esti = X0init(:, 2);
ysim2 = sim(nlgr, z2.InputData, simOptions('InitialCondition', x02_esti));
[metric_fit_val,metric_mse_val] = helper_func_new_metrics_by_traj(z2.OutputData, ysim2);


z3 = getexp(z, 3);                 % experiment 3 only
x03_esti = X0init(:, 3);
ysim3 = sim(nlgr, z3.InputData, simOptions('InitialCondition', x03_esti));
[metric_fit_z3,metric_mse_z3] = helper_func_new_metrics_by_traj(z3.OutputData, ysim3);



z4 = getexp(z, 4);                 % experiment 4 only
x04_esti = X0init(:, 4);
ysim4 = sim(nlgr, z4.InputData, simOptions('InitialCondition', x04_esti));
[metric_fit_test,metric_mse_test] = helper_func_new_metrics_by_traj(z4.OutputData, ysim4);




disp(metric_fit_train);
disp(metric_fit_val);
disp(metric_fit_test);
disp(metric_fit_z3);

% disp(metric_mse_train);
% disp(metric_mse_test);

Matlab_grey = struct;
Matlab_grey.u_train_batch = z1.InputData;
Matlab_grey.y_train_batch = z1.OutputData;

Matlab_grey.u_val_batch = z2.InputData;
Matlab_grey.y_val_batch = z2.OutputData;

Matlab_grey.u_test_batch = [z3.InputData z4.InputData];
Matlab_grey.y_test_batch = [z3.OutputData z4.OutputData];



Matlab_grey.y_pre_train_batch_cl = ysim1;
Matlab_grey.y_pre_val_batch_cl = ysim2;
Matlab_grey.y_pre_test_batch_cl = [ysim3 ysim4];

Matlab_grey.y_pre_train_batch = ysim1;
Matlab_grey.y_pre_val_batch = ysim2;
Matlab_grey.y_pre_test_batch = [ysim3 ysim4];


save(fullfile(results_dir, 'Data_Exp2_Greybox.mat'), 'Matlab_grey');
%%%%%%%%%%%%%%%%%%%%%%%% modification end %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Jeff: my functions
function [fit_vec, mse_vec] = local_metrics_by_traj(y_true_mat, y_pred_mat)
    % Input dimensions:
